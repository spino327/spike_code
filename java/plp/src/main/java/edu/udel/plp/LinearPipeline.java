...
public abstract class LinearPipeline {
    ...
    /**
     * Create desired array of pipeline stage objects.
     * @param args
     * @return
     */
    public abstract PipelineStage[] getPipelineStages (Object... args);
    
    /**
     * Create desired array of BlockingQueues
     * element i of returned array contains queue between stages i and i+1
     * @param args
     * @return
     */
    public abstract BlockingQueue[] getQueues (Object... args);
    
    public LinearPipeline (Object... args) {
        stages = getPipelineStages(args);
        numStages = stages.length;
        queues = getQueues(args);
        cdl = new CountDownLatch(numStages);
        
        BlockingQueue in = null;
        BlockingQueue out = queues[0];
        for (int i = 0; i < numStages; i++) {
            stages[i].init(in, out, cdl);
            in = out;
            out = i < numStages - 2 ? queues[i+1]
                    : null;
        }
    }
    ...
    public void start () {
        for (int i = 0; i < numStages; i++)
            new Thread(stages[i], stages[i].getClass().getName()).start();
    }
}
